...
export default function DreamEditModal({ dream, onClose, onSave, saving, lang = 'tr' }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
      <div className="relative w-full max-w-2xl max-h-[92dvh] overflow-y-auto overscroll-contain pb-8 rounded-[2rem] border border-white/10 bg-[#060912]/95 p-6 shadow-[0_0_80px_rgba(139,92,246,0.18)]">
          ...
          <div className="mb-4 rounded-[1.2rem] border border-white/10 bg-black/30 p-4">
            ...
            {image?.url ? (
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 rounded-xl border border-dashed border-white/15 py-5 text-sm text-slate-400 hover:border-brand-primary-400/40 hover:text-brand-primary-200 transition-all"
              >
                <ImageIcon size={16} />
            ...
            {image?.url && (
              <button
                onClick={() => setShowPixabayPicker(true)}
                className="mt-2 text-xs text-brand-primary-300 hover:text-brand-primary-200"
              >
