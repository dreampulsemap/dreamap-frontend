...
/**
 * KOLEKTIF (anonim) baglam — SADECE toplu istatistik.
 *
 * GIZLILIK: Bu metin tum kullanicilarin ruyalarindan uretilip HERKESE ayni
 * sekilde gosteriliyor. Bu yuzden buraya ruya ICERIGI konulamaz; tek bir
 * kullanicinin ozel metni (ad, e-posta, kisisel detay) baskalarinin ekranina
 * dusebilir. Yalnizca arketip/duygu sayimlari aktariliyor.
 */
export function buildCollectiveContext(dreams = []) {
  ...
  for (const dream of dreams) {
    if (Array.isArray(dream.ai_archetypes)) {
      for (const arch of dream.ai_archetypes) {
  ...
  const archetypeName = topArchetype ? topArchetype[0] : 'Shadow'
  const percentage = totalArchetypes > 0
    ? Math.round(((topArchetype ? topArchetype[1] : 0) / totalArchetypes) * 100)
    : 0
  ...
  return {
    ...
    goalCount: 0,
    collective: true,
    text: `Across ${dreams.length} anonymous dreams shared in the last week, the dominant archetype is ${archetypeName} (${percentage}%) and the prevailing emotion is ${topEmotion ? topEmotion[0] : 'Mystery'}.`
  }
}

function userPrompt({ mode, question, context }) {
  if (context.collective) {
    return `${context.text}\n\nWrite one short collective prophecy for the whole dreaming community, based only on the summary above. Do not address a single person.`
  }
  if (mode === 'ask') {
