...
export async function generateOneImage(prompt) {
  let imageUrl = null
  let replicateDetails = 'Unknown error'
  ...
  try {
    const rep = await fetch('https://api.replicate.com/v1/models/black-forest-labs/flux-schnell/predictions', {
      ...
      body: JSON.stringify({ input: { prompt, aspect_ratio: '9:16' } }),
    })
    const data = await rep.json()
    if (data.output) imageUrl = data.output[0]
    else replicateDetails = data.detail || JSON.stringify(data)
  } catch (e) {
    replicateDetails = e.message
  }

  // PLAN B: OpenAI DALL-E 3 (Fallback)
  if (!imageUrl) {
    try {
      const image = await openai.images.generate({ model: 'dall-e-3', prompt, n: 1, size: '1024x1792' })
      imageUrl = image.data[0].url
    } catch (e) {
      // KÖK NEDEN NOTU: burada önceden sadece OpenAI'nin hatası dönüyordu,
      // Replicate'in NEDEN başarısız olduğu (details) sessizce atılıyordu —
      // /api/goals/generate-cover 502 döndüğünde Vercel loglarında hiçbir
      // ipucu kalmıyordu (bkz. "AI Kapak Üret 502" hata raporu). İkisini de
      // birlikte döndürüyoruz ki çağıran taraf console.error ile loglayabilsin.
      return { imageUrl: null, details: `replicate: ${replicateDetails} | openai: ${e.message}` }
    }
  }
