...
export function useModalA11y(containerRef, onClose) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      //
      // ERTELEME (KÖK NEDEN — "Düzenle geri atıyor" hatası): Bir modal
      // kapanırken YERİNE aynı render batch'i içinde başka bir modal
      // açılıyorsa (ör. SlidesViewer kapanıp GoalDetailModal açılması,
      // "Düzenle" akışı), bu cleanup çalıştığı anda yığın geçici olarak
      // boş görünür ama birazdan (aynı efekt flush'ı içinde) yeni modalin
      // mount efekti yığını tekrar dolduracaktır. Burada senkron
      // history.back() çağırırsak, o çağrı ASENKRON olarak biraz sonra
      // gerçek bir popstate'e dönüşür — ve o noktada yığının tepesinde
      // artık YENİ açılan modal vardır; popstate onu (yanlışlıkla) kapatır.
      // Sonuç: "Düzenle"ye basınca yeni sayfa bir an açılıp kendi kendine
      // kapanıyormuş gibi görünür. Çözüm: kararı bir mikrotask'a erteleyip
      // o an yığın hâlâ boş mu diye TEKRAR kontrol ediyoruz — yeni modal
      // bu arada mount olup yığını doldurduysa geri gitmiyoruz.
      if (!entry.closedByPopState) {
        queueMicrotask(() => {
          if (modalStack.length === 0) window.history.back()
