...
async function sendFcmToUser(supabaseAdmin, userId, payload) {
  ...
  await Promise.all(
    tokens.map(async ({ token }) => {
      try {
        // Bilerek "notification" bloğu GÖNDERMİYORUZ, sadece "data" —
        // FCM'de notification+data birlikte gidince, uygulama arka
        // plandayken/kapalıyken sistem bildirimi OTOMATİK gösteriyor ve
        // LunosferMessagingService.onMessageReceived() hiç çalışmıyor. Bu da
        // WhatsApp tarzı kişi-bazlı gruplama/inline-reply/okundu-işaretle
        // mantığının (ve mevcut deep-link yönlendirmesinin) arka planda
        // sessizce devre dışı kalması demekti. Data-only mesajlar uygulama
        // durumundan bağımsız HER ZAMAN onMessageReceived()'i tetikler.
        // "android.priority: high" olmadan data-only mesajlar cihaz
        // uykudayken gecikebilir/ertelenebilir.
