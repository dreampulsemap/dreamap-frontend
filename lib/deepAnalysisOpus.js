...
// 5 dakikalik bir pencerede tekrar kullanilir.
// =====================================================================

export const OPUS_MODEL = 'claude-opus-5'
export const DEEP_ANALYSIS_AURA_COST = 10

// Opus 5 fiyatlandirmasi, 1M token basina USD.
const PRICE_INPUT = 5
...
export const SUPPORTED_LANGS = ['en', 'tr', 'ru', 'ar', 'es', 'hi', 'zh', 'de', 'fr', 'pt', 'ja']
...
export async function generateDeepAnalysis({ lang, dreams, goals, archetypes }) {
  ...
  try {
    ...
    // Model bazen JSON'u aciklayici bir cumleyle sariyor; ilk { ile son }
    // arasini kurtarmayi dene.
    const first = cleaned.indexOf('{')
    const last = cleaned.lastIndexOf('}')
    if (first === -1 || last <= first) throw new Error('invalid_json_from_model')
