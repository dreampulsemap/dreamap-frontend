...
export async function getAcceptedFriendIds(userId) {
  ...
  // Arkadasliklar CIFT YONLU saklaniyor: ayni cift icin hem (A,B) hem (B,A)
  // satiri var. Tekillestirme olmadan bu fonksiyon her arkadasi IKI kez
  // donduruyordu; diary/feed.js bunu ring listesine cevirdigi icin Android
  // tarafindaki LazyRow ayni userId'yi iki kez key olarak aliyor ve
  // "Key ... was already used" ile uygulamayi ACILISTA cokertiyordu
  // (canli cihazda goruldu). Kendi id'sini de disariyoruz: bozuk/self
  // arkadaslik satiri varsa kullanici kendi halkasini iki kez gorurdu.
  const ids = (data || []).map((f) => (f.user_id === userId ? f.friend_id : f.user_id))
  return [...new Set(ids)].filter((id) => id && id !== userId)
}
