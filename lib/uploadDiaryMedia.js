...
// Bucket PRIVATE: getPublicUrl() artik erisilemeyen bir adres uretiyor ve
// composer'daki onizleme bos kaliyordu. Imzali URL donuyoruz. Bu deger
// DB'ye de yaziliyor, sakincasi yok — sunucu okuma aninda yoldan yeni bir
// imza uretiyor (bkz. lib/diaryMediaUrl.js).
const SIGNED_URL_TTL_SECONDS = 60 * 60 * 24

async function signedMediaUrl(filePath) {
...
export async function uploadDiaryMedia({ file, userId, onProgress }) {
  ...
  if (isImage) {
    ...
    const signedUrl = await signedMediaUrl(filePath)
    onProgress?.({ phase: 'done', percent: 100 })
    return { url: signedUrl, mediaType: 'photo', posterUrl, ...dimensions }
  }
  ...
  if (posterBlob) {
    try {
      ...
      if (!posterError) {
        posterUrl = await signedMediaUrl(posterPath)
      }
    } catch {
  ...
  const signedVideoUrl = await signedMediaUrl(videoPath)
  onProgress?.({ phase: 'done', percent: 100 })
  return { url: signedVideoUrl, mediaType: 'video', posterUrl, width: null, height: null }
}
