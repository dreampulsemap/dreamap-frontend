...
export const auth = {
  ...
  // Şifre sıfırlama e-postası gönderir. Kullanıcı e-postadaki linke
  // tıklayınca Supabase bir "recovery" oturumu açıp aşağıdaki sayfaya
  // yönlendirir; şifre orada updatePassword ile değiştirilir.
  ...
  async signInWithOAuth(providerOrConfig, redirectToArg) {
    let provider = 'google'
    ...
    if (typeof providerOrConfig === 'string') {
      ...
      provider = providerOrConfig.provider || 'google'
      redirectTo = providerOrConfig.options?.redirectTo
    }
  ...
  async ensureProfile(user, overrides = {}) {
    ...
    if (error) {
      console.error(
        'Profil oluşturulamadı/güncellenemedi:',
        error.message
      )
      return null
    }
