...
function buildShape() {
  return {
    ...
    motiv: '',
    sentiment: '',
    visual_prompt_en: '', // YENİ EKLENEN ÖZEL PROMPT ALANI
    archetypes: [],
    shadow_focus: '',
    core_conflict: '',
    individuation_path: '',
...
The entire JSON values MUST be written entirely in: ${targetLangName}, EXCEPT for the "visual_prompt_en" field which MUST be in English.
...
- "visual_prompt_en": Write a highly detailed, cinematic English prompt for an AI image generator (like Midjourney/Flux) focusing on the most striking, surreal, and specific scene described in the dream. Include subjects, actions, lighting, and mood. DO NOT use the word 'dream'. Only write the prompt string.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const analysis = parseJsonSafely(completion?.choices?.[0]?.message?.content)
    if (!analysis) throw new Error('Invalid JSON from model')

    // =========================================================================
    // GÖRSEL ÜRETİMİ VE KALICI KAYIT
    // =========================================================================
    ...
    try {
      ...
      if (replicateRes.ok && replicateData?.output?.[0]) {
        ...
        // GÖRSELİ İNDİR VE SUPABASE'E YÜKLE
        try {
          ...
          if (imageFetchRes.ok) {
            ...
            const { error: uploadError } = await supabaseAdmin.storage
              .from('dream_images') // Açtığınız bucket
            ...
            if (!uploadError) {
              ...
              finalImageUrl = tempImageUrl; // Hata olursa geçici url'yi kullan
            }
          }
        } catch (storageError) {
      ...
      console.error('Replicate error:', imageError)
    }
