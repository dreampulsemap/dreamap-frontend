import { supabaseAdmin, getAuthedUser } from '@/lib/supabaseAdmin'

export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'method_not_allowed' })

  try {
    const user = await getAuthedUser(req)
    if (!user) return res.status(401).json({ error: 'unauthorized' })

    const { goalId, imageUrl } = req.body || {}
    if (!goalId || typeof imageUrl !== 'string' || !imageUrl.trim()) {
      return res.status(400).json({ error: 'invalid_params' })
    }

    const { data: goal, error: fetchError } = await supabaseAdmin
      .from('goals')
      .select('id, user_id, gallery_image_urls')
      .eq('id', goalId)
      .single()

    if (fetchError || !goal) return res.status(404).json({ error: 'goal_not_found' })
    if (goal.user_id !== user.id) return res.status(403).json({ error: 'not_owner' })

    const existing = Array.isArray(goal.gallery_image_urls) ? goal.gallery_image_urls : []
    const updated = existing.filter((url) => url !== imageUrl)

    const { data: updatedGoal, error: updateError } = await supabaseAdmin
      .from('goals')
      .update({ gallery_image_urls: updated })
      .eq('id', goalId)
      .select('*')
      .single()

    if (updateError) throw updateError

    // Storage'daki dosyayı da sessizce silmeyi dene — başarısız olsa da
    // isteği başarısız saymıyoruz (görsel zaten galeriden kaldırıldı).
    try {
      const bucketPrefix = '/storage/v1/object/public/goal-images/'
      const idx = imageUrl.indexOf(bucketPrefix)
      if (idx !== -1) {
        const path = imageUrl.slice(idx + bucketPrefix.length)
        await supabaseAdmin.storage.from('goal-images').remove([path])
      }
    } catch (storageErr) {
      console.error('goals/remove-image storage cleanup error:', storageErr)
    }

    return res.status(200).json({ goal: updatedGoal, gallery_image_urls: updated })
  } catch (error) {
    console.error('goals/remove-image error:', error)
    return res.status(500).json({ error: error.message || 'internal_error' })
  }
}
